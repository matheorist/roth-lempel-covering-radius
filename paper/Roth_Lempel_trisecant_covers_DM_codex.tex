...
\]
are supplied in the companion supplementary table file in the public
repository.  For \(q\in\{19,23,29,31,37\}\), each listed class includes an
explicit first-hole certificate.  The full class-level results for
\(q=7,8,9\) are contained in the exact small-prime-power log.  Exact
certificate logs for the later fields \(q=41,43,47,49,53\) and the
extension fields \(q=16,25,27,32\) are listed in
Section~\ref{sec:reproducibility} and are available in the public repository
above.
