...
\begin{abstract}
Covering radii and deep-hole syndromes are determined for all length-eight,
dimension-four Roth--Lempel codes \(C=\RL_{4,\delta}(S)\) with \(|S|=6\).
Every such code is MDS or NMDS, and it is NMDS precisely when \(\delta\) is
the sum of three distinct elements of \(S\).  The main result is the sharp
threshold
\[
  q\ge16 \quad\Longrightarrow\quad \rho(C)=4 .
\]
Together with the small-field classification, this gives a complete answer
for this family over all prime powers \(q\ge7\).
Below the threshold, the radius-three codes are classified completely up to
affine equivalence: their numbers are \(2,3,14,32,7\) over
\(\F_7,\F_8,\F_9,\F_{11},\F_{13}\), respectively, and no other
radius-three case exists.  The proof converts \(\rho(C)\le3\) into a
finite-geometric covering condition by the \(56\) planes spanned by triples
of parity-check columns in \(\PG(3,q)\); an uncovered point is simultaneously
a deep-hole syndrome and a projection center producing a plane \(8\)-arc.
An explicit twisted-cubic construction proves the threshold for every
\(q\ge23\), and for every NMDS member already when \(q\ge19\), leaving only
thirty residual affine representatives over \(q=16,17,19\) to finite
certification.  For \(q\ge16\), all deep-hole syndromes are characterized by
the nonvanishing of one explicit \(56\)-factor polynomial.  The distance
stratification is also explicit in every admissible field, with closed
relations among the projective syndrome shells, including
\(m_2(C)=28(q-1)-3r_S(\delta)\).
\end{abstract}

\noindent\textbf{Keywords:}
Roth--Lempel codes; covering radius; deep holes; MDS codes; NMDS codes;
finite geometry; affine classification.
...
Roth--Lempel codes are a classical source of non-Reed--Solomon MDS
codes~\cite{RothLempel1989}.
...
criteria and weight distributions for Roth--Lempel
codes~\cite{HanFan2023,XuZhou2026}; Liang and Liao construct further NMDS
classes from the Roth--Lempel matrix and study extended generalized
Roth--Lempel codes~\cite{LiangLiao2026,LiangLiaoDM2026}.  In parallel,
covering radii and deep holes have become structural tools for extended
twisted-GRS and non-GRS MDS--NMDS
constructions~\cite{LiZhuSun2025,LiLuLingLam2026}.  Distinct from these works,
this paper determines the complete covering-radius threshold and explicit
deep-hole syndrome criterion for the fixed length-eight, dimension-four
Roth--Lempel family.
...
\(8\)-arcs (Lemma~\ref{lem:projection}).
...
admissible field, and the projective shell sizes satisfy closed relations in
terms of \(q\), \(r_S(\delta)\), and \(m_4(C)\).  In particular, the second
shell is
\[
 m_2(C)=28(q-1)-3r_S(\delta),
\]
...
Thus the paper gives a complete structure theorem for the classical
Roth--Lempel family of MDS and NMDS codes.  The result also supplies an
infinite collection of certified \([8,4]_q\) Roth--Lempel seed codes with
maximal covering radius and
explicitly decidable deep-hole syndromes for deep-hole-based extension
constructions; see Corollary~\ref{cor:seed-codes}.  The finite part of the
proof uses exact Magma enumeration, not randomized search.  The
proof-critical computations, certificates, and independent cross-checks are
separated and documented in Section~\ref{sec:reproducibility}.
...
rank-two projection of two distinct evaluation columns onto the first two
coordinates, or from the nonzero first coordinate of a single evaluation column together
...
\begin{lemma}\label{lem:second-shell}
...
By Lemma~\ref{lem:duality}, every dependent four-set of parity-check
points has the form
\[
...
cover \(\PG(3,q)\).  The final assertion is the standard redundancy
bound~\cite{Cohen1997,Huffman2003}.
\end{proof}
...
next therefore give a finite-geometric witness for the existence of such
projected arcs in the non-exceptional range.

\begin{proposition}
\label{prop:explicit-product}
Let \(C=\RL_{4,\delta}(S)\), let
\(\theta=\sum_{a\in S}a-\delta\), and choose homogeneous coordinates
\(P=(p_0,p_1,p_2,p_3)\) for a projective syndrome relative to the
parity-check matrix in Lemma~\ref{lem:duality}.  Define
\[
\begin{aligned}
A_a(P)&=p_1-ap_0,\\
...
\begin{proof}
By Lemma~\ref{lem:duality}, column scaling does not affect the
...
\(4\) if and only if \(x\notin\mathcal{B}_{S,\delta}\).  Let \(m_4(C)\)
denote the number of projective syndromes at distance \(4\), and let
\(r=r_S(\delta)\).  Then
\[
  \lvert\mathcal{B}_{S,\delta}\rvert\le21-3r,\qquad
  m_4(C)\ge q-21+3r.
\]
Consequently:
\[
...
\[
 \lvert\mathcal{B}_{S,\delta}\rvert\le 6+(15-3r)=21-3r.
\]
There are therefore at least \(q-21+3r\) twisted-cubic centers whenever
this number is positive.  The projective-syndrome assertions follow from
Proposition~\ref{prop:explicit-product}.  Each projective syndrome coset
contains \(q^4(q-1)\) words, giving the stated lower bound for deep-hole
words.
\end{proof}
...
\[
 ...
 \mathcal{B}_{S',\delta'}=u\mathcal{B}_{S,\delta}+v.
\]
...
\]
and the asserted transformation of \(\mathcal{B}_{S,\delta}\) follows.
\end{proof}

\begin{remark}\label{rem:boundary-families}
...
certificates used later, but their elementary count does not improve the
...
layers proves the displayed stratification.  Finally, the trisecant cover is
complete in a radius-three class, so only the first three layers remain.
\end{proof}

\begin{lemma}
\label{lem:deep-census}
...
Lemma~\ref{lem:second-shell} gives \(m_2(C)\), and
\[
 m_1(C)+m_2(C)+m_3(C)+m_4(C)=q^3+q^2+q+1.
\]
Solving for \(m_3(C)\) gives the stated vector.  If \(\rho(C)=3\), then
\(h=0\), and Lemma~\ref{lem:deep-census} converts \(m_3(C)\) to
the number of deep-hole words.
\end{proof}

\begin{corollary}\label{cor:large-q}
...
\begin{proposition}\label{prop:large-q-refined}
...
\]
all pairs \((S,\delta)\) with \(|S|=6\) were enumerated and reduced under
the affine action from Section~2.  For each affine representative, the
covering radius was computed directly or radius \(4\) was certified by an
explicit uncovered syndrome point for \(\RL_{4,\delta}(S)\).  This
normalization is the natural affine
...
\(7\le q\le53\) are summarized in Table~\ref{tab:complete-classification}.
No class in these classifications has radius below \(3\).
\begin{center}
\captionof{table}{Complete affine classifications for
\(\RL_{4,\delta}(S)\), \(|S|=6\), over \(7\le q\le53\).}
\label{tab:complete-classification}
\scriptsize
\(
...
class occurs.  For \(q=11,13\), where radius-three classes persist, every affine
representative is constructed in Magma and the trisecant-plane cover is
computed directly; the covering radius computation was also run as an
independent check.  For \(q=16\), all \(536\) affine representatives are
certified by the two-slice search, with full-space fallback for the residual
classes.  For \(q=17,19,23,29,31,37\), where no hole-free class occurs, the
scripts record an explicit uncovered projective syndrome point for every
affine representative.  The additional scans for
\(q=25,27,32,41,43,47,49,53\) first test one or two structured projective
...
not integer encodings.  No class remains uncertified.
Proposition~\ref{prop:criterion} then gives \(\rho=4\) for every certified
...
Thus the boundary fields \(q=16,17,19\) are covered in two independent
ways: the proof of Theorem~\ref{thm:threshold} uses the twisted-cubic
reduction plus thirty explicit residual certificates, while the complete
enumeration in Theorem~\ref{thm:complete} supplies an independent
class-by-class computational verification.
...
matrix \(H\) in Lemma~\ref{lem:duality}.  Then \(u\) is a deep hole
of \(C\) if and only if
\[
  \Phi_{S,\delta}(P)\ne0.
\]
...
\begin{corollary}\label{cor:seed-codes}
For every prime power \(q\ge16\), the length-eight Roth--Lempel family
\(\RL_{4,\delta}(S)\), \(|S|=6\), supplies explicit MDS or NMDS
\([8,4]_q\) seed codes with maximal covering radius.  More precisely, for
each seed the deep-hole syndrome cosets are exactly the nonzero syndromes
\(P\) satisfying \(\Phi_{S,\delta}(P)\ne0\).  Therefore any
deep-hole-based extension framework whose hypotheses require an explicit
deep hole in such a seed, such as the framework
in~\cite{LiLuLingLam2026}, may choose \(P\) by this nonvanishing test.
\end{corollary}

\begin{proof}
This is an immediate restatement of Theorem~\ref{thm:threshold} and
Corollary~\ref{cor:explicit-deepholes}.  The MDS/NMDS alternative is
Proposition~\ref{prop:exhaustion}.
\end{proof}

The three smallest fields have particularly compact exceptional
descriptions.  Here \(\F_8=\F_2(w)\), \(w^3+w+1=0\), and
\(\F_9=\F_3(w)\), \(w^2+2w+2=0\).  The compact representatives are listed
in Table~\ref{tab:small-exceptional}; all displayed classes have \(\rho=3\).
\begin{center}
\captionof{table}{Compact representatives for the radius-three classes over
\(q=7,8,9\).}
\label{tab:small-exceptional}
\(
...
9 & \mathrm{NMDS} & \{0,1,w,w^2,w^3,2\} & \delta\in\F_9 & 9\\
...
Together with Table~\ref{tab:complete-classification}, this shows that
every affine class over \(q=7\) and \(q=9\) has radius \(3\), whereas
radius-four classes already occur over \(q=8\) and \(q=11\).
...
given in Table~\ref{tab:r-spectrum}.
\begin{center}
\captionof{table}{Low-field radius spectrum by the multiplicity
\(r_S(\delta)\).}
\label{tab:r-spectrum}
\small
\begin{tabular}{c c r r@{\qquad}c c r r}
\toprule
\(q\)&\(r_S(\delta)\)&\(\rho\)&classes&
\(q\)&\(r_S(\delta)\)&\(\rho\)&classes\\
\midrule
7&2&3&1  & 11&0&4&1\\
7&3&3&1  & 11&1&3&14\\
...
In contrast, Lemma~\ref{lem:second-shell} shows that this same
additive statistic determines the entire second syndrome shell exactly.

Lemma~\ref{lem:deep-census} also turns the exceptional classification
into an exact deep-hole census.  Table~\ref{tab:dh-coset-spectrum} gives
the complete spectrum for the classes with \(\rho=3\); ``DH cosets'' denotes
\((q-1)m_3(C)\), and every such coset contains \(q^4\) deep-hole words.
\begin{center}
\captionof{table}{Deep-hole coset spectrum for the radius-three classes.}
\label{tab:dh-coset-spectrum}
\small
\begin{tabular}{c c r r r@{\qquad}c c r r r}
\toprule
\(q\)&type&DH cosets&classes&raw&
\(q\)&type&DH cosets&classes&raw\\
\midrule
...
code type and by the number of uncovered projective syndrome points is shown
in Table~\ref{tab:q11-q13-hole-distribution}.
\begin{center}
\captionof{table}{Hole distribution for \(q=11\) and \(q=13\).}
\label{tab:q11-q13-hole-distribution}
\scriptsize
\begin{tabular}{c c r r@{\qquad}c c r r}
\toprule
\multicolumn{4}{c}{\(q=11\)}&
\multicolumn{4}{c}{\(q=13\)}\\
\cmidrule(r{1.2em}){1-4}\cmidrule(l){5-8}
...
For \(q=17,19,23,29,31,37\), Table~\ref{tab:negative-mds-nmds} records the
MDS/NMDS split in fields where no radius-three class occurs.
\begin{center}
\captionof{table}{MDS/NMDS classification for fields with no radius-three
classes.}
\label{tab:negative-mds-nmds}
\small
\begin{tabular}{c c r r r r}
\toprule
\(q\)&\(\rho\)&MDS classes&MDS raw&NMDS classes&NMDS raw\\
\midrule
...
\end{tabular}
\end{center}
No affine class is hole-free for \(q=17,19,23,29,31,37\).  For \(q=17\),
where full
...
An earlier one-slice certificate computation, which searches first in the
structured projective slice \(P=[1,u,v,v]\), is retained in the
reproducibility manifest as a historical cross-check.  The stronger
two-slice scan is the compact form reported in the article.
...
It handles both prime and non-prime fields.  As shown in
Table~\ref{tab:two-slice-certificates}, the final column vanishes in every
field listed.
\begin{center}
\captionof{table}{Two-slice certificate search for prime and extension
fields.}
\label{tab:two-slice-certificates}
\(
...
representative families, as in Table~\ref{tab:q11-radius-three-families}.
\begin{center}
\captionof{table}{Compact representative families for the \(q=11\)
radius-three classes.}
\label{tab:q11-radius-three-families}
\(
\begin{array}{c|c|c}
\hbox{type} & S & \delta\\
\hline
...
\)
\end{center}
These rows account for \(32\) affine classes; the full \(q=11\) affine list
is given in Section~\ref{sec:q11-complete}.

\begin{corollary}\label{cor:q13-seven}
Over \(\F_{13}\), exactly the seven affine classes in
Table~\ref{tab:q13-seven-classes} have covering radius \(3\).
\begin{center}
\captionof{table}{The seven radius-three affine classes over \(\F_{13}\).}
\label{tab:q13-seven-classes}
\(
\begin{array}{c|c|c|c}
...
pairs, have \(\rho=4\).  The complete list is given in
Table~\ref{tab:q11-complete}, and Table~\ref{tab:q11-hole-summary}
summarizes the same classification by type, radius, and number of holes.

\begin{center}
\captionof{table}{Summary of the \(q=11\) affine classification by type,
radius, and holes.}
\label{tab:q11-hole-summary}
\(
...
The set labels used in Table~\ref{tab:q11-complete} are
\[
  \begin{array}{ll}
...
\]
\begin{center}
\captionof{table}{Complete affine classification over \(\F_{11}\) for \(k=4,n=6\).}
\label{tab:q11-complete}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{r r c c c r r@{\qquad}r r c c c r r}
\toprule
idx&orb&type&\(\rho\)&\(S\)&\(\delta\)&holes&
idx&orb&type&\(\rho\)&\(S\)&\(\delta\)&holes\\
\midrule
...
\end{tabular}
\end{center}
Here ``idx'' and ``orb'' denote the class index and affine-orbit size,
respectively.

\noindent\textbf{Companion supplementary tables.}
\begin{sloppypar}
...
repository.  For \(q=19,23,29,31,37\), each listed class includes an explicit
first-hole certificate.  The full class-level results for \(q=7,8,9\) are
contained in the exact small-prime-power log.  Exact certificate logs for
the remaining certified fields are listed in Section~\ref{sec:reproducibility}
and are available in the public repository cited there.
\end{sloppypar}
...
In the language of Lemma~\ref{lem:projection}, the classes with
...
The complete classifications show that this cover property occurs throughout
...
already when \(q\ge19\).  Consequently
Theorem~\ref{thm:threshold} is a complete statement for the entire
length-eight family: a covering-radius drop can occur only below \(q=16\),
and it occurs exactly in the classes enumerated over \(q=7,8,9,11,13\).
...
Lemma~\ref{lem:second-shell} shows that it completely controls the
second shell.  Lemma~\ref{lem:affine-covariance} also ensures that
...
For reference, Table~\ref{tab:boundary-certificate-families} gives the
exact evaluation of the displayed certificate families on the three boundary
fields and on \(q=23\), the first field covered uniformly by the
twisted-cubic criterion.
\begin{center}
\captionof{table}{Boundary-field certificate counts for the displayed
center families.}
\label{tab:boundary-certificate-families}
\(
...
Extending the same syndrome-geometric method beyond length eight is a natural
direction for future work.

\section{Reproducibility}\label{sec:reproducibility}

All finite computations reported in this paper were performed in
Magma~\cite{Magma1997}, using exact finite-field arithmetic.  Parameter pairs are
...
\href{https://github.com/matheorist/roth-lempel-covering-radius/blob/main/%
certificates/supplement.tex}
{available in the public repository}.
For every \(q\geq 23\), Theorem~\ref{thm:threshold} is theoretical and does
not depend on an exhaustive machine scan.  The complete-classification scans
reported in Theorem~\ref{thm:complete}, including the \(q=16\) two-slice
scan, provide independent class-by-class checks of these conclusions rather
than additional hypotheses for the threshold proof.

\subsection*{Verification summary}
Table~\ref{tab:verification-summary} summarizes the verification tasks used
by the proof and separates proof-critical computations from cross-checks.
\begin{center}
\captionof{table}{Verification tasks and their logical role.}
...
Boundary class-level cross-checks &
\(16,17,19\) &
Complete affine-representative scans, including the \(q=16\) two-slice
scan and explicit uncovered syndromes for \(q=17,19\).\\
Formula validation &
\(7,\ldots,19\) &
...
{program-and-log reproducibility manifest} records the
...
\bibitem{LiangLiaoDM2026}
Z. Liang, Q. Liao,
``The extended code for a class of generalized Roth--Lempel codes and their
properties,''
\emph{Discrete Math.}, vol. 349, Art.\ no. 115084, 2026,
doi:10.1016/j.disc.2026.115084.
