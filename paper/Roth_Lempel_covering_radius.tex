...
codes~\cite{HanFan2023,XuZhou2026}; Liang and Liao construct further NMDS classes
...
\(8\)-arcs (Lemma~\ref{prop:projection}).
...
Roth--Lempel family of MDS and NMDS codes.  The result also supplies an infinite collection
of certified
\([8,4]_q\) Roth--Lempel seed codes with maximal covering radius and
...
rank-two projection of two distinct evaluation columns onto the first two coordinates, or
...
\begin{lemma}\label{prop:duality}
...
Lemma~\ref{prop:duality}.
\end{proof}

\begin{lemma}\label{prop:second-shell}
...
By Lemma~\ref{prop:duality}, every dependent four-set of parity-check
points has the form
\[
...
\begin{lemma}\label{prop:projection}
...
parity-check matrix in Lemma~\ref{prop:duality}.  Define
\[
\begin{aligned}
A_a(P)&=p_1-ap_0,\\
...
\begin{proof}
By Lemma~\ref{prop:duality}, column scaling does not affect the
...
Lemma~\ref{prop:second-shell} gives \(m_2(C)\), and
\[
 m_1(C)+m_2(C)+m_3(C)+m_4(C)=q^3+q^2+q+1.
\]
Solving for \(m_3(C)\) gives the stated vector.  If \(\rho(C)=3\), then
\(h=0\), and Lemma~\ref{prop:deep-census} converts \(m_3(C)\) to
...
matrix \(H\) in Lemma~\ref{prop:duality}.  Then \(u\) is a deep hole
of \(C\) if and only if
\[
  \Phi_{S,\delta}(P)\ne0.
\]
...
In contrast, Lemma~\ref{prop:second-shell} shows that this same
additive statistic determines the entire second syndrome shell exactly.

Lemma~\ref{prop:deep-census} also turns the exceptional classification
...
For \(q=17,19,23,29,31,37\), Table~\ref{tab:negative-mds-nmds} shows that
the classification by MDS/NMDS type is purely negative.
\begin{center}
...
In the language of Lemma~\ref{prop:projection}, the classes with
...
Lemma~\ref{prop:second-shell} shows that it completely controls the
second shell.  Lemma~\ref{prop:affine-covariance} also ensures that
